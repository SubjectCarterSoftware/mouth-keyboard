...
| 03-02-01 | 02 | 2 | SESS-01 | unit | `xcodebuild test -only-testing Speech2TestTests/SpacebarInterceptorTests` | W0 | pending |
...
- [ ] `Speech2TestTests/SpacebarInterceptorTests.swift` — stubs for SESS-01 (spacebar interception logic via protocol mock, not actual CGEventTap)
...
|----------|-------------|------------|-------------------|
| Spacebar swallowed during recording (not typed into active app) | SESS-01 | CGEventTap requires Accessibility permission; cannot run in headless CI | 1. Start recording via hotkey 2. Press spacebar 3. Verify no space character appears in active text field 4. Verify recording stops and transcription begins |
...
| Auto-paste into active application | CLIP-01 | CGEvent Cmd+V requires Accessibility permission | 1. Open TextEdit 2. Dictate via Speech2Test 3. Verify text appears at cursor position after finish |
