...
- [ ] **SESS-01**: User can finish the active recording with the spacebar while remaining in the current application.
...
- [ ] **TRNS-01**: User receives local-first speech transcription for captured audio.
- [ ] **TRNS-02**: User receives punctuation in the transcribed text without manual cleanup for normal dictation.
...
- [ ] **TRNS-06**: User receives a clear failure state instead of a silent or misleading success when transcription cannot produce usable text.

### Clipboard Output

- [ ] **CLIP-01**: User receives the final transcription in the system clipboard after a successful session.
...
- [ ] **FEED-01**: User can distinguish idle, recording, and processing states from visual indicators.
- [ ] **FEED-02**: User can keep the app in the background while using any other application during dictation.
- [ ] **FEED-03**: User can choose whether the recording indicator is visible.
