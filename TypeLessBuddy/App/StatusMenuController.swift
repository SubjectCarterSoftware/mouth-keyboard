...
final class StatusMenuController: NSObject, NSMenuDelegate {
    ...
    func updateIcon(for state: RecordingState) {
        ...
        case .processing:
            pillColor = .systemBlue
            description = "Processing"
        case .modelDownloading:
            pillColor = .systemCyan
        ...
        statusItem?.button?.image = menuBarWaveform(
            pillColor: pillColor,
    ...
    private func menuBarWaveform(pillColor: NSColor?, accessibilityDescription: String) -> NSImage? {
        let canvasSize = NSSize(width: 22, height: 18)
        let symbolRect = NSRect(x: 2, y: 0, width: 18, height: 18)
        let baseSymbolConfig = NSImage.SymbolConfiguration(pointSize: 18, weight: .regular)

        guard let symbol = NSImage(
            systemSymbolName: "waveform",
            accessibilityDescription: accessibilityDescription
        )?.withSymbolConfiguration(baseSymbolConfig) else {
            return nil
        }
        ...
        let image = NSImage(size: canvasSize, flipped: false) { _ in
            // Match the full menu bar icon footprint so colored states do not appear shrunken.
            let pillRect = NSRect(x: 0, y: 0, width: canvasSize.width, height: canvasSize.height)
            let pillPath = NSBezierPath(roundedRect: pillRect, xRadius: 9, yRadius: 9)
            pillColor?.setFill()
            pillPath.fill()

            let whiteSymbolConfig = NSImage.SymbolConfiguration(pointSize: 18, weight: .semibold)
                .applying(NSImage.SymbolConfiguration(paletteColors: [.white]))
            guard let whiteSymbol = NSImage(
                systemSymbolName: "waveform",
                accessibilityDescription: accessibilityDescription
            )?.withSymbolConfiguration(whiteSymbolConfig) else {
                return false
            }

            whiteSymbol.isTemplate = false
