...
struct StoreQuarantine {
    static func quarantine(storeURL: URL, label: String) {
        ...
        do {
            ...
            NSLog("Speech2Text: Quarantined corrupt \(label) store to \(quarantinedURL.path)")
        } catch {
            NSLog("Speech2Text: Failed to quarantine corrupt \(label) store at \(storeURL.path): \(error.localizedDescription)")
        }
    }
}
