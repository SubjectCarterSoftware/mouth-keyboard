...
- `TypeLessBuddy/Activation/TriggerTranscriptParser.swift`
- `TypeLessBuddy/Activation/ActivationStore.swift`
- `TypeLessBuddy/Conversion/LLMRewriteService.swift`
- `TypeLessBuddy/Shell/SetupWindowView.swift`
...
In `TypeLessBuddy/Activation/ActivationStore.swift`, where assistant-triggered rewrite currently calls the split rewrite path, replace it with:
...
That copy appears in `TypeLessBuddy/Shell/SetupWindowView.swift`.
...
This can live beside the existing rewrite prompt logic, likely in `TypeLessBuddy/Conversion/LLMRewriteService.swift` or a small new helper file.
...
In `TypeLessBuddy/Activation/ActivationStore.swift`:
...
Revise onboarding and assistant activation explanation in `TypeLessBuddy/Shell/SetupWindowView.swift`.
